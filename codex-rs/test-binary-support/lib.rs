    ...
    F: FnOnce(&str, Option<&str>) -> TestBinaryDispatchMode,
{
    ...
    match classify(exe_name, argv1.as_deref().and_then(|arg| arg.to_str())) {
        ...
        TestBinaryDispatchMode::InstallAliases => {
            ...
            let previous_codex_home = std::env::var_os("CODEX_AKRAM_HOME");
            // Safety: this runs from a test ctor before test threads begin.
            unsafe {
                std::env::set_var("CODEX_AKRAM_HOME", codex_home.path());
            }
            ...
            match previous_codex_home.as_ref() {
                Some(value) => unsafe {
                    std::env::set_var("CODEX_AKRAM_HOME", value);
                },
                None => unsafe {
                    std::env::remove_var("CODEX_AKRAM_HOME");
                },
            }
