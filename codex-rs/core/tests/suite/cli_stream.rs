...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = codex_utils_cargo_bin::cargo_bin("codex-akram").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(repo_root())
        .arg("hello?");
    cmd.env("CODEX_AKRAM_HOME", home.path())
...
// Use this for new `codex-akram exec` subprocess tests in this file. These commands
...
/// Verify that `codex-akram exec --profile ...` preserves the active user config
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = codex_utils_cargo_bin::cargo_bin("codex-akram").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("CODEX_AKRAM_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");
