...
#[command(
    version,
    override_usage = "codex-akram exec [OPTIONS] [PROMPT]\n       codex exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Do not load `$CODEX_AKRAM_HOME/config.toml`; auth still uses `CODEX_AKRAM_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "codex-akram resume --last <prompt>" treats the positional as a prompt, not a session id.
