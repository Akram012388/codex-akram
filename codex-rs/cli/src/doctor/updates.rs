...
const GITHUB_LATEST_RELEASE_URL: &str = codex_akram_identity::GITHUB_LATEST_RELEASE_API;
...
pub(super) fn updates_check(config: &Config) -> DoctorCheck {
    ...
    match fetch_latest_version(&install_context) {
        Ok(latest_version) => {
            ...
            if is_newer(&latest_version, codex_akram_identity::VERSION) == Some(true) {
...
fn update_action_label(_context: &InstallContext) -> &'static str {
    "codex-akram standalone installer"
}
...
mod tests {
    ...
    fn is_newer_compares_plain_semver() {
        ...
        assert_eq!(is_newer("0.145.0-ak.0.1", "0.145.0-ak.0.2"), Some(false));
    }
}
