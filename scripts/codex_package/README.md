...
`codex-akram`, `codex`, and `codex-app-server`. The default is `codex-akram`.
Its package version comes from `codex-rs/akram-identity/Cargo.toml`; upstream
variants retain the workspace version from `codex-rs/Cargo.toml`.
