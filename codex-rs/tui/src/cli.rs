...
pub struct Cli {
    ...
    // Internal controls set by the top-level `codex-akram resume` subcommand.
    ...
    /// top-level `codex-akram resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `codex-akram fork` subcommand.
    // These are not exposed as user flags on the base `codex` command.
    #[clap(skip)]
    ...
    /// top-level `codex-akram fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
