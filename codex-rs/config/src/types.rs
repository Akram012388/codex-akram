...
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in CODEX_AKRAM_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in CODEX_AKRAM_HOME.
    Auto,
...
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// CODEX_AKRAM_HOME/.credentials.json
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$CODEX_AKRAM_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against CODEX_AKRAM_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
