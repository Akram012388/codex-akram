...
pub unsafe fn protect_workspace_codex_dir(cwd: &Path, psid: *mut c_void) -> Result<bool> {
    let protected_codex = protect_workspace_subdir(cwd, psid, ".codex")?;
    let protected_akram = protect_workspace_subdir(cwd, psid, ".codex-akram")?;
    Ok(protected_codex || protected_akram)
}
